...
const TradeList: React.FC<TradeListProps> = ({ trades, userProfile, onAddTrade, onEditTrade, onDeleteTrade, onDeleteAll, onPreviewTrade }) => {
  ...
  return (
    <div className="space-y-4 h-full flex flex-col relative pb-20 md:pb-0">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-slate-800 p-4 rounded-xl border border-slate-700 z-20 relative gap-4 md:gap-0">
        <div className="relative w-full md:w-auto">
          <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-500 w-4 h-4" />
          <input
            type="text"
            placeholder="Search symbol or strategy..."
            className="bg-slate-900 border border-slate-700 text-slate-200 pl-10 pr-4 py-2 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 w-full md:w-64"
            value={filter}
        ...
        <div className="flex gap-2 relative" ref={filterPanelRef}>
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm transition-colors border ${isFilterPanelOpen || activeFilterCount > 0
                  ...
                  <div className="flex bg-slate-900 p-1 rounded-lg border border-slate-700">
                    {['ALL', Direction.LONG, Direction.SHORT].map(opt => (
                      <button
                        ...
                        className={`flex-1 py-1 text-xs rounded-md transition-colors font-medium ${activeFilters.direction === opt
                  ...
                  <div className="flex bg-slate-900 p-1 rounded-lg border border-slate-700">
                    {['ALL', 'WIN', 'LOSS'].map(opt => (
                      <button
                        ...
                        className={`flex-1 py-1 text-xs rounded-md transition-colors font-medium ${activeFilters.result === opt
                    ...
                    <div className="absolute top-full left-0 right-0 mt-1 bg-slate-900 border border-slate-700 rounded-lg shadow-xl z-50 max-h-48 overflow-y-auto custom-scrollbar">
                      {['ALL', ...Object.values(AssetClass)].map((option) => (
                        <button
                          ...
                          className={`w-full text-left px-3 py-2 text-xs transition-colors border-l-2 ${(option === 'ALL' && activeFilters.assetClass === 'ALL') || activeFilters.assetClass === option
      ...
      {/* Mobile Trade List (Cards) */}
      <div className="flex-1 overflow-auto md:hidden space-y-3 pb-20">
        {sortedTrades.map(trade => {
          const violations = getTradeViolations(trade, trades, userProfile);
          const hasViolations = violations.length > 0;
          return (
            <div key={trade.id} onClick={() => onPreviewTrade && onPreviewTrade(trade)} className="bg-slate-800 p-4 rounded-xl border border-slate-700 shadow-sm active:bg-slate-700/50 transition-colors">
              <div className="flex justify-between items-start mb-2">
                <div className="flex flex-col">
                  <span className="text-xs text-slate-500 font-mono mb-1">{new Date(trade.entryDate).toLocaleDateString()}</span>
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-slate-200 text-lg">{trade.symbol}</span>
                    {trade.images && trade.images.length > 0 && <ImageIcon className="w-3 h-3 text-slate-500" />}
                  </div>
                </div>
                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${trade.direction === Direction.LONG ? 'bg-emerald-900/40 text-emerald-400' : 'bg-rose-900/40 text-rose-400'
                  }`}>
                  {trade.direction}
                </span>
              </div>
              ...
              <div className="flex justify-between items-center mb-3">
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] text-slate-500">{trade.setup}</span>
                </div>
                <div className="text-right">
                  <div className={`font-mono font-bold ${trade.netPnL > 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                    {formatCurrency(trade.netPnL, currency)}
                  </div>
                  <div className={`text-xs font-mono ${trade.riskMultiple > 0 ? 'text-emerald-500/70' : 'text-rose-500/70'}`}>
              ...
              <div className="flex justify-between items-center pt-3 border-t border-slate-700/50">
                <div className="flex items-center gap-2">
                  {hasViolations && <AlertTriangle className="w-4 h-4 text-amber-500" />}
                </div>
                <div className="flex gap-3">
                  <button
                    onClick={(e) => { e.stopPropagation(); onEditTrade && onEditTrade(trade); }}
                    className="p-2 bg-slate-700 rounded-lg text-slate-300 hover:text-white"
                  >
                    <Edit2 className="w-4 h-4" />
                  </button>
                  <button
                    onClick={(e) => { e.stopPropagation(); onDeleteTrade && onDeleteTrade(trade.id); }}
                    className="p-2 bg-rose-900/20 rounded-lg text-rose-400 hover:text-rose-300 hover:bg-rose-900/40"
                  >
        ...
        {trades.length === 0 && (
          <div className="text-center py-12 text-slate-500 flex flex-col items-center">
            <ListIcon className="w-12 h-12 mb-2 opacity-20" />
            <p>No trades logged yet.</p>
          </div>
        )}
      </div>

      <div className="hidden md:block flex-1 overflow-auto bg-slate-800 rounded-xl border border-slate-700 shadow-inner z-10">
            ...
            {sortedTrades.map(trade => {
              ...
              return (
                <tr key={trade.id} className="hover:bg-slate-700/30 transition-colors group">
                  ...
                  <td className="p-4">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${trade.direction === Direction.LONG ? 'bg-emerald-900/30 text-emerald-400' : 'bg-rose-900/30 text-rose-400'
                      }`}>
                      {trade.direction}
                    </span>
                  </td>
